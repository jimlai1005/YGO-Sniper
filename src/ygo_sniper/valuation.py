...
⚠️ **上面那一組「Mercari 高估」的實測數字帶著一層量測污染（2026-08-06 實測，
重估平台係數之前要先解決）**：comps 表裡 Mercari 的成交時間**全部是假的**。
...
Buyee 系的已售出頁根本不含成交日期，`comps.py` 於是退回 `now()` 並標上
`sold_at_is_ingest=1`（欄位定義見 `store.py`）。後果是**任何以時間為基礎、
涉及 Mercari 的統計，切的其實是入庫順序不是時間順序**：把 Mercari 的
era-verified 成交按 `sold_at` 排序再切 75/25，兩段的中位價是 5,624 → 1,818
（×0.32）。那不是市場在三個月內跌了三分之二，是**我們的抓取順序**——
訓練段與測試段的商品組成本來就不一樣，而它被讀成了時間。

受影響的路徑：`_time_split`（因此 `coverage_time_split`、`coverage_by_group`
都在內）。`_time_split` 的 `stratify_by_venue=True` 只擋住「早晚被切成平台」
這一半的問題，**擋不住平台內部的入庫順序污染**。所以 `L3 × Mercari` 那一格
的 100% 下尾違反率，是「模型真的高估」與「訓練／測試被入庫順序切出組成差異」
兩者的**合計**，兩個成分用現有資料分不開。

這對兩件事的影響不同，不要混為一談：
  - **拒絕給上限**（`reject_n_buckets`）：不受影響。拒絕的成本是輸掉競標，
    而污染只會讓這個保守動作更保守（「讀不到就不要下注」）。
  - **「Mercari 高估 6-8 倍」這個幅度**：**不可當精確值引用**，也不可拿來
    當重新校準平台係數的依據。要重估係數，先讓 Mercari 有真的成交時間
    （或只用 `coverage-time --real-time-only` 那條 Yahoo 乾淨路徑）。
"""
...
class Obs:
    ...
    `sold_at` 只給時間切分的覆蓋率自檢用——模型本身不看時間（視窗過濾是 store
    那一層的決定）。

    ⚠️ **`sold_at` 不保證是成交時間**：Buyee 系的已售出頁不給日期，那批列存的是
    我們的入庫時間（2026-08-06 實測：Mercari 1046/1046 筆、PayPay 77/563 筆；
    Yahoo 落札相場 0 筆，它有真的成交日）。詳情與後果見模組頂註最後一段。

    `Obs` **刻意不帶 `sold_at_is_ingest` 旗標**，因為那一層的決定不在這裡：
    來源列（`comps.*`）每一筆都帶著旗標，要不要排除是**取樣端**的選擇
    （`ygo-sniper coverage-time --real-time-only` 就是在 `obs_from_comps`
    之前把那些列濾掉）。把旗標帶進 `Obs` 卻沒有任何一支函式讀它，只會製造
    「看起來有處理」的假象——那比沒有更糟。要在模型層做分流時，這個決定要
    連同「哪一支函式因此改變行為」一起做，不要先加欄位。
...
def _time_split(
    ...
    ⚠️ **分層只擋住一半，剩下一半沒有解（2026-08-06 實測）**：Mercari 那一格
    自己內部的「早／晚」仍然是**入庫先後**，不是成交先後（1046 筆全部如此）。
    實測按 `sold_at` 切 75/25，Mercari 兩段的中位價是 5,624 → 1,818（×0.32）
    ——那是抓取順序造成的組成差異，會被下游讀成「市場漂移」。所以只要樣本裡
    有 Buyee 系的列，這支函式回的就**不是**乾淨的時間切分；要乾淨的，在呼叫
    `obs_from_comps` 之前就把 `sold_at_is_ingest=1` 的列濾掉
    （`ygo-sniper coverage-time --real-time-only` 走的就是那條路）。
    這裡不自己濾：`Obs` 沒有帶旗標（理由見 `Obs` 的 docstring），而且「要不要
    為了乾淨而砍掉四成樣本」是取樣端的取捨，不是切分函式該偷偷替人做的決定。
    """
...
def coverage_by_group(
    ...
    *,
    ...
    ⚠️ **時間切分本身帶著污染，讀任何 Mercari／PayPay 相關的格子之前要先知道**：
    這支函式靠 `_time_split` 切早晚，而 Buyee 系的 `sold_at` 是入庫時間
    （2026-08-06 實測：Mercari 1046/1046 筆），所以那些平台切到的是入庫順序。
    「拒絕給上限」這種保守動作不受影響（見模組頂註的分項說明），但**逐格的
    數字不可當成精確的模型偏誤量測**。`ygo-sniper coverage-groups` 會把當下
    有幾筆是入庫時間印在表格上方，不要跳過那一行。
    """
