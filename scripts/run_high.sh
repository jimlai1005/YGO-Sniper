#!/bin/bash
# ---------------------------------------------------------------------------
# 高價帶（¥8,624～50,000）掃描的實際入口。launchd 跑這個
# （scripts/com.jim.ygosniper.high.plist，白天偶數整點＋晚間 :15）。
# 手動也可以跑這個。與低價帶的 run_daily.sh 各自獨立：鎖、log、排程監督
# 帳本都分開，兩條掃描允許並行、互不搶。
#
...
#   4. 需要防重入 —— 見下面的鎖
#
# 排程監督（`data/last_run_exit_high`）的事故背景，以及 watchdog／孤兒視窗的
# 細節分析，都在 docs/run_daily.md（低價帶那份文件同樣適用於這支腳本的
# 機制設計）——這支腳本本身只留「這裡在做什麼」，不留「為什麼」的長篇分析。
# ---------------------------------------------------------------------------
...
# log 一天一個檔，14 天輪替邏輯照舊可用
LOG_FILE="$LOG_DIR/high-$(date +%Y%m%d).log"
LOCK_DIR="$PROJECT_DIR/data/run_high.lock"
# 排程監督帳本：這一輪結束時（不論成功失敗）覆寫一次，讓下一輪的
# `ygo-sniper daily-high` 讀得到「上一輪到底是怎麼結束的」。細節見
# write_last_run_ledger 與 docs/run_daily.md。獨立檔名——不能跟低價帶的
# data/last_run_exit 共用，否則兩條掃描的排程監督會互相覆寫對方的帳本。
LAST_RUN_FILE="$PROJECT_DIR/data/last_run_exit_high"
...
# ---------------------------------------------------------------------------
# 防重入。一輪掃描要開 Playwright（chromium），遇到 WAF 重取 token 或網路慢
# 時可能跑超過一個時段，下一輪就會疊上來——兩個 chromium 同時搶記憶體、
# 同時打同一個站（更容易被擋）、還會對同一批 signal 重複推播。
#
...
if ! acquire_lock; then
    ...
    # 回非零會讓下面那段失敗通知每個時段發一則「掃描失敗」，把真正的故障淹掉。
...
# NOTIFY_* 全域變數讓 write_last_run_ledger 讀——下一輪 `ygo-sniper daily-high`
...
if [ ! -f ".venv/bin/activate" ]; then
    ...
    notify_failure "⚠️ ygo-sniper 高價帶找不到 .venv（請先跑 make setup），本輪整個沒執行到掃描，請看 data/logs/"
...
echo "===== $(date '+%Y-%m-%d %H:%M:%S') 開始（高價帶） =====" >> "$LOG_FILE"
...
.venv/bin/python scripts/run_with_timeout.py "${YGO_CYCLE_TIMEOUT:-1500}" \
    ygo-sniper daily-high >> "$LOG_FILE" 2>&1
STATUS=$?

if [ $STATUS -ne 0 ]; then
    echo "[$(date '+%H:%M:%S')] 高價帶掃描失敗，exit=$STATUS" >> "$LOG_FILE"
    if [ "$STATUS" -eq 124 ]; then
        MSG="🚨 ygo-sniper 高價帶本輪被 watchdog 強制終止（超過 ${YGO_CYCLE_TIMEOUT:-1500}s），可能 Playwright 卡死，請看 data/logs/"
    else
        MSG="⚠️ ygo-sniper 高價帶掃描失敗 (exit=${STATUS})，請看 data/logs/"
    fi
...
echo "===== $(date '+%H:%M:%S') 結束（高價帶，exit=$STATUS） =====" >> "$LOG_FILE"

write_last_run_ledger "$STATUS"

# 只留最近 14 天的 log
find "$LOG_DIR" -name 'high-*.log' -mtime +14 -delete 2>/dev/null
